...
The document serves as the primary technical reference for the development team, QA testers, project managers, and university stakeholders (secretariat staff and faculty). It establishes a shared understanding of expected system behavior, external interfaces, performance targets, and security constraints before and during implementation.

\subsection{Document Conventions}
The following conventions apply throughout this document:
\begin{itemize}
    \item Headings use Times New Roman, size 14, bold (per the document template).
    \item Body text uses Arial, size 11.
    \item ``Shall'' indicates mandatory system behavior; ``should'' indicates recommended behavior.
    \item Functional requirements use structured IDs (e.g., REQ-XX), non-functional requirements use NFR-XX, and code quality rules use CODE-XX.
    \item Bold text highlights key terms when first introduced.
    \item Bullet points and lists organize requirements for readability.
\end{itemize}

\subsection{Intended Audience and Reading Suggestions}
This SRS targets a range of stakeholders. Each audience should prioritize sections relevant to their role:
\begin{itemize}
    \item \textbf{Registrar and Secretariat Staff (Primary):} Focus on Section 2.2 (Product Functions) and Section 3.1 (User Interfaces) to verify data entry optimization and Microsoft Office integration.
    \item \textbf{External Visitors and Prospective Students:} Refer to Section 2.2 and Section 3.1.1 (Public Web Interface) for accessible academic data (curricula, announcements) that require no authentication.
    \item \textbf{Software Developers and Technical Architects:} Analyze Section 3 (Specific Requirements) and Section 4 (System Features) for functional mandates and technical constraints.
    \item \textbf{System Administrators:} Focus on Section 5.2 (Safety Requirements) for rollback protocols and Section 5.3 (Security Requirements) for authentication and audit log management.
    \item \textbf{Academic Staff (Professors):} Review Section 2.2 (Product Functions) and Section 3.1 (Web Portal) for grade entry and group communication features.
    \item \textbf{Quality Assurance (QA) and Testing Teams:} Examine Section 3 for verifiable requirements and Section 6 (Other Requirements) for testing mandates.
\end{itemize}

\subsection{Product Scope}
The product specified here is the Automated Faculty Student Management System (AFSMS / GASF).

\textbf{Objectives and Benefits}
AFSMS digitalizes and optimizes the faculty's administrative and educational workflows, supporting the university's goals of digital transformation and operational efficiency. Key benefits:
\begin{itemize}
    \item Eliminates data redundancy and manual entry errors via client-side validation and centralized database management.
    \item Reduces processing time for the registrar's office when generating official documents (e-Transcript, e-Grade Centralizer).
    \item Provides 24/7 information transparency for students and staff via a secure, role-based web portal.
\end{itemize}
...
\begin{itemize}
    ...
    \item Standardized report generation, exportable in CSV, XML, XLS, and PDF formats.
    \item Native integration with Microsoft Excel and Microsoft Outlook.
...
\begin{itemize}
    ...
    \item University of Craiova (ACE), Academic Curricula and Educational Plans: https://ace.ucv.ro/invatamant/didactica/planuri.php
    \item University of Craiova (ACE), Student Formations and Timetable Schedules: https://ace.ucv.ro/invatamant/utile/orar.php
    \item University of Craiova (ACE), Institutional Evaluation and Grading Procedures: https://ace.ucv.ro/invatamant/utile/modalitati_de_evaluare.php
...
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{2.4cm} >{\raggedright\arraybackslash}p{1.2cm} >{\raggedright\arraybackslash}p{1.8cm} >{\raggedright\arraybackslash}X}
\toprule
...
The following items remain to be determined during the detailed design and planning phases:
...
\begin{itemize}
    ...
    \item \textbf{TBD-4 (NFR-AFSMS-QUAL-021):} Exact module boundaries to ensure extensibility without negative effects.
